        ...
        <tbody>
            @foreach ($purchaseRequest->items as $index => $item)
                @php
                    ...
                    if ($item->rejected_finance_at) {
                        $rowClass = 'table-danger';
                        $rejectDate = date('d M Y', strtotime($item->rejected_finance_at));
                        $statusText = "Ditolak oleh {$item->rejected_finance_by} ({$rejectDate}). Alasan: {$item->rejection_finance_note}";
                    }
                @endphp
                <tr class="{{ $rowClass }}">
                    ...
                    <td class="text-center">
                        @if ($item->attachment && file_exists(public_path('purchase-requests/' . $item->attachment)))
                            <img src="{{ public_path('purchase-requests/' . $item->attachment) }}" class="img-preview"
                                alt="Lampiran">
                        @else
