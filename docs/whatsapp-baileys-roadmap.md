...
> **Status:** Fase 3 Selesai � Fase 4 Sedang Dikerjakan  
...
| Backend Node di Lumen | `LUMEN-V3/javascript/` | **Kosong � siap diisi** |
| Integrasi Baileys | � | **Belum ada** |
...
+-------------------------------------------------------------+
�  React Frontend (ERP)                                       �
�  d:\JAVASCRIPT\REACT-JS\frontend                            �
�                                                             �
�  MessagesDropdown --? WhatsAppPanel (WA Web UI)             �
�       �                     �                               �
�       �                     +-- QRConnect                   �
�       �                     +-- ChatList                      �
�       �                     +-- ChatWindow                  �
+-------------------------------------------------------------+
                � HTTP (token Lumen)       � Socket.io + REST
                ?                          ?
+---------------------------+   +-------------------------------+
�  Lumen PHP API            �   �  wa-service (Node.js)         �
�  d:\PHP\LUMEN-V3          �   �  javascript/wa-service/       �
�                           �   �                               �
�  POST /api/cektoken       �?--�  lumenAuth.js (validasi)     �
�  POST /api/route          �   �  Baileys sessionManager       �
�  WaProxyController (ops)  �   �  Socket.io realtime           �
+---------------------------+   +-------------------------------+
                �                               �
                +-------------------------------+
                                ?
...
> **Catatan:** Jika nanti dibutuhkan 1 nomor WA perusahaan (shared), arsitektur session manager perlu disesuaikan � bukan scope MVP.

---

## 5. Struktur Folder

### 5.1 Backend � `LUMEN-V3/javascript/wa-service/`
...
�   +-- auth/
�   �   +-- lumenAuth.js         # Validasi token ke Lumen /api/cektoken
�   +-- baileys/
�   �   +-- sessionManager.js    # Map userId ? Baileys socket instance
�   �   +-- qrHandler.js         # Generate & emit QR ke frontend
�   �   +-- eventHandlers.js     # messages.upsert, connection.update, contacts.upsert
�   +-- services/
�   �   +-- messageService.js    # CRUD pesan, send text/media
�   �   +-- contactService.js    # Sync & enrich kontak
�   �   +-- mediaService.js      # Download/upload media WA
�   +-- socket/
�   �   +-- waSocket.js          # Socket.io rooms per userId
�   +-- routes/
�   �   +-- api.js               # REST endpoints
�   +-- db/
�       +-- connection.js        # MySQL pool (shared config Lumen)
�       +-- queries/             # Raw queries atau Sequelize models
...
### 5.2 Frontend � `frontend/src/modules/whatsapp/`

```
src/modules/whatsapp/
+-- WhatsAppPanel.jsx            # Panel utama (entry dari ListKontak / MessagesDropdown)
+-- components/
�   +-- QRConnect.jsx            # Layar scan QR (mirip WA Web)
�   +-- ChatList.jsx             # Sidebar kiri: daftar chat
�   +-- ChatWindow.jsx           # Area chat kanan
�   +-- MessageBubble.jsx        # Bubble pesan (teks, media, status)
�   +-- MessageInput.jsx         # Input teks + attach file
�   +-- ContactAvatar.jsx        # Avatar kontak/grup
�   +-- ConnectionStatus.jsx     # Indicator online/offline/connecting
�   +-- MediaPreview.jsx         # Preview sebelum kirim / lightbox gambar
+-- hooks/
�   +-- useWaSocket.js           # Socket.io connection + events
�   +-- useWaSession.js          # Status session (qr/connected/disconnected)
�   +-- useWaMessages.js         # Load, send, paginate messages
+-- services/
�   +-- waApi.js                 # REST calls ke wa-service
+-- store/
    +-- waSlice.js               # Redux: session, chats, messages, unreadTotal
```

### 5.3 Migration PHP � `LUMEN-V3/database/migrations/`
...
- [x] `sessionManager.js` � Map `userId ? Baileys socket`, load/save auth state dari `sessions/{userId}/`
- [x] `qrHandler.js` � Handle `connection.update`, emit QR string ke Socket.io
- [x] `eventHandlers.js` � Handle `creds.update` (persist session), `connection.update` (status)
- [x] `lumenAuth.js` � Middleware REST + Socket.io handshake: validasi token ke `POST /api/cektoken`
- [x] `waSocket.js` � Room per `userId`, events: `wa:qr`, `wa:status`, `wa:connected`, `wa:disconnected`
- [x] REST endpoints:
  - `GET /api/wa/status` � Status koneksi user
  - `POST /api/wa/logout` � Hapus session & disconnect
  - `GET /api/wa/qr` � Fallback polling QR (opsional)
...
### Fase 2 � Frontend QR + Connection UI
...
- [x] `useWaSocket.js` � Connect Socket.io dengan token ERP dari localStorage
- [x] `useWaSession.js` � State machine: `disconnected ? qr ? connecting ? connected`
- [x] `QRConnect.jsx` � Tampilkan QR image, instruksi link device, loading spinner
- [x] `ConnectionStatus.jsx` � Badge status di header panel
- [x] `WhatsAppPanel.jsx` � Router internal: QR screen vs Chat screen
- [x] Integrasi `MessagesDropdown.js` � Klik ikon ? buka panel WA fullscreen/overlay
- [x] Refactor `ListKontak.js` � Render `WhatsAppPanel` atau redirect
- [x] Redux `waSlice.js` � State session dasar
...
### Fase 3 � Chat List + Pesan Teks Realtime
...
- [ ] `messageService.js`:
  - `getChats(userId)` � Daftar chat + last message + unread
  - `getMessages(chatId, cursor, limit)` � Pagination
  - `sendText(userId, jid, text)` � Kirim via Baileys
  - `markRead(userId, chatId)` � Reset unread
...
- [ ] `ChatList.jsx` � Sidebar: avatar, nama, preview, waktu, badge unread, search
- [ ] `ChatWindow.jsx` � Header chat, area bubble, scroll auto-bottom
- [ ] `MessageBubble.jsx` � Bubble kiri/kanan, timestamp, status icon (?/??)
- [ ] `MessageInput.jsx` � Textarea + tombol kirim (Enter = send)
- [ ] `useWaMessages.js` � Load, send, realtime append
...
### Fase 4 � Media: Gambar, Video, File, Dokumen
...
- [ ] `sendMedia(userId, jid, file, type)` � image, video, document, audio
...
- [ ] `contactService.syncAll(userId)` � Pull dari Baileys store
...
### 9.1 wa-service � `javascript/wa-service/.env`
...
| Fase | Durasi | Hasil |
|------|--------|-------|
| **0** � Persiapan | 1�2 hari | Scaffold + DB ready |
| **1** � Baileys core | 3�4 hari | QR connect backend |
| **2** � QR UI | 2�3 hari | QR connect frontend |
| **3** � Chat teks | 4�5 hari | Messaging realtime |
| **4** � Media | 3�4 hari | Gambar, video, file |
| **5** � Kontak | 2 hari | Sync kontak |
| **6** � Polish | Ongoing | Fitur lanjutan |
| | | |
| **MVP (Fase 0�3)** | **~10�14 hari** | Chat teks fungsional |
| **Full (Fase 0�5)** | **~15�20 hari** | Mirip WA Web |
...
| GET | `/media/*` | � | Serve file media static |
