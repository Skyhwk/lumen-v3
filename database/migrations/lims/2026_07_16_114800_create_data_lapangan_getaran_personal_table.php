<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateDataLapanganGetaranPersonalTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up(): void
    {
        Schema::create('data_lapangan_getaran_personal', function (Blueprint $table) {
            $table->id();
            $table->string('no_sampel', 20)->nullable();
            $table->text('no_sampel_lama')->nullable();
            $table->integer('kategori_3')->nullable();
            $table->text('keterangan')->nullable();
            $table->text('keterangan_2')->nullable();
            
            // Geokursi & Metode
            $table->string('titik_koordinat', 30)->nullable();
            $table->string('latitude', 30)->nullable();
            $table->string('longitude', 30)->nullable();
            $table->string('metode', 30)->nullable();
            $table->string('sumber_getaran', 100)->nullable();
            $table->string('posisi_penguji', 25)->nullable();
            
            // Durasi & Kondisi
            $table->json('durasi_paparan')->nullable();
            $table->string('durasi_kerja', 10)->nullable();
            $table->string('kondisi', 50)->nullable();
            $table->text('pengukuran')->nullable();
            $table->json('bobot_frekuensi')->nullable();
            
            // Titik Paparan Tubuh
            $table->text('tangan')->nullable();
            $table->text('pinggang')->nullable();
            $table->text('betis')->nullable();
            
            // Parameter & Satuan Pengukuran
            $table->string('durasi_pengukuran', 20)->nullable();
            $table->string('satuan_percepatan', 10)->nullable();
            $table->string('satuan_kecepatan', 10)->nullable();
            $table->string('satuan_kecepatan_x', 10)->nullable();
            $table->string('satuan_kecepatan_y', 10)->nullable();
            $table->string('satuan_kecepatan_z', 10)->nullable();
            $table->string('satuan_kecepatan_aeq', 10)->nullable();
            $table->string('intensitas', 20)->nullable();
            $table->string('waktu_pengukuran', 50)->nullable();
            
            // Data Subjek & Alat
            $table->string('nama_pekerja', 100)->nullable();
            $table->string('jenis_pekerja', 100)->nullable();
            $table->string('lokasi_unit', 50)->nullable();
            $table->text('alat_ukur')->nullable();
            $table->string('posisi_pengukuran', 50)->nullable();
            $table->string('adaptor', 50)->nullable();
            
            // Berkas Gambar
            $table->string('foto_lokasi_sampel', 50)->nullable();
            $table->string('foto_lain', 50)->nullable();
            
            // Workflow Status & Audit Trails
            $table->string('created_by', 70)->nullable();
            $table->timestamp('created_at')->nullable();
            $table->integer('permission')->default(0);
            $table->integer('is_blocked')->default(0);
            $table->string('blocked_by', 70)->nullable();
            $table->timestamp('blocked_at')->nullable();
            $table->integer('is_approve')->default(0);
            $table->string('approved_by', 70)->nullable();
            $table->timestamp('approved_at')->nullable();
            $table->tinyInteger('is_rejected')->default(0);
            $table->string('rejected_by', 70)->nullable();
            $table->timestamp('rejected_at')->nullable();
            $table->timestamp('updated_at')->nullable();
            $table->string('updated_by', 70)->nullable();

            // Indexing
            $table->index('no_sampel', 'idx_dlgp_no_sampel');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('data_lapangan_getaran_personal');
    }
}
