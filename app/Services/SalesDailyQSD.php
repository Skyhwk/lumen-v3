...
class SalesDailyQSD
{
    ...
    private static function handle(int $currentYear): bool
    {
        ...
        if ($result->isNotEmpty()) {
            ...
            DB::statement("
                UPDATE daily_qsd dq
                INNER JOIN order_header oh ON oh.no_order = dq.no_order
                SET dq.tanggal_order = oh.tanggal_order
    ...
    private static function getYearRange(int $currentYear): array
    {
        ...
        $baseYear = (int)Carbon::create($currentYear, 12, 1)->subYear(1)->endOfMonth()->format('Y');
        $arrayYears = [];
        for ($i = $baseYear; $i <= $nextYear; $i++) {
            $arrayYears[] = $i;
        }
        return $arrayYears;
    }
