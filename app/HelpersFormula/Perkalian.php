...
class Perkalian
{
    public function index($data, $id_parameter, $mdl)
    {
        ...
        $is_5_decimal = [3, 4, 22, 23, 24, 36, 37, 42, 43, 49, 50, 77, 78, 100, 101, 112, 113, 155, 156, 157, 53, 54, 190, 189, 4, 3, 167, 166, 2398, 2397];
        ...
        if (strpos($data->hp, '<') !== false || strpos($data->hp, '>') !== false || !is_numeric($data->hp)) {
            // dd('masuk');
            $rumus = $data->hp;
        } else {
            // dd('masuk1');
            $rumus = in_array($id_parameter, $not_decimal) ? str_replace(',', '', $data->hp * $data->fp) : str_replace(',', '', number_format($data->hp * $data->fp, $decimal));
