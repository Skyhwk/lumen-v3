...
class AppsSenyawaVolatileController extends Controller
{
    public function getSample(Request $request)
    {
        if (isset($request->no_sampel) && $request->no_sampel != null) {
            ...
            if (is_null($data)) {
                ...
                $senyawa = DataLapanganSenyawaVolatile::where('no_sampel', strtoupper(trim($request->no_sampel)))->first();
                if ($senyawa !== NULL) {
                    \DB::statement("SET SQL_MODE=''");
                    $param = DetailSenyawaVolatile::where('no_sampel', strtoupper(trim($request->no_sampel)))->groupBy('parameter')->get();
                    $parNonSes = array();
                    foreach ($param as $value) {
                        if ($value->shift_pengambilan != 'Sesaat') {
                            $p = DetailSenyawaVolatile::where('no_sampel', strtoupper(trim($request->no_sampel)))->where('parameter', $value->parameter)->get();
    ...
    public function getShift(Request $request){
        $data = DetailSenyawaVolatile::where('no_sampel', $request->no_sampel)->where('shift_pengambilan', $request->shift)->first();
        $po = OrderDetail::where('no_sampel', $request->no_sampel)->whereIn('kategori_3', ['11-Udara Ambient', '27-Udara Lingkungan Kerja'])->where('is_active', true)->first();
        \DB::statement("SET SQL_MODE=''");

        $param = DetailSenyawaVolatile::where('no_sampel', $request->no_sampel)->groupBy('parameter')->get();
        $parNonSes = array();


        foreach ($param as $value) {
            if ($value->kategori_pengujian != 'Sesaat') {
                $p = DetailSenyawaVolatile::where('no_sampel', $request->no_sampel)->where('parameter', $value->parameter)->get();
        ...
        $lh_params_check = DetailLingkunganHidup::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', 'L1')
            ->pluck('parameter')
            ->toArray();

        $lk_params_check = DetailLingkunganKerja::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', 'L1')
            ->pluck('parameter')
            ->toArray();

        $voc_params_check = DetailSenyawaVolatile::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', 'L1')
            ->pluck('parameter')
            ->toArray();
        ...
        $lh_parameter = DetailLingkunganHidup::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', $final_shift)
            ->pluck('parameter')
            ->toArray();

        $lk_parameter = DetailLingkunganKerja::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', $final_shift)
            ->pluck('parameter')
            ->toArray();

        $voc_parameter = DetailSenyawaVolatile::where('no_sampel', $request->no_sampel)
            ->where('shift_pengambilan', $final_shift)
            ->pluck('parameter')
            ->toArray();
        ...
        try {
            ...
            if ($request->param != null) {
                foreach ($request->param as $en => $ab) {
                    $cek = DetailSenyawaVolatile::where('no_sampel', strtoupper(trim($request->no_sampel)))->where('parameter', $ab)->get();
            ...
            foreach ($request->param as $in => $a) {
                ...
                $fdl = DataLapanganSenyawaVolatile::where('no_sampel', strtoupper(trim($request->no_sampel)))->first();
                // if (is_null($fdl)) {
            ...
            $nama = $this->karyawan;
            $this->resultx = "Data Sampling Senyawa Volatile Dengan No Sample $request->no_sampel berhasil disimpan oleh $nama";
