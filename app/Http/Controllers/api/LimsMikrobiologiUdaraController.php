...
class LimsMikrobiologiUdaraController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $data = MicrobioHeader::with('ws_value')
            ->join(DB::raw("{$limsDb}.order_detail as order_detail"),'order_detail.no_sampel','=','microbio_header.no_sampel')
            ->where('is_approved', $request->is_approved)
            ->where('microbio_header.is_active', true)
            ->where('template_stp', $request->template_stp)
            ->select('microbio_header.*', 'order_detail.tanggal_terima as od_tanggal_terima', 'order_detail.kategori_3 as od_kategori_3', 'order_detail.tanggal_sampling as os_tanggal_sampling')
            ->orderByRaw("
                CASE 
        ...
        if ($request->filled('periode')) {
            ...
            if (count($periode) == 2) {
                $data->whereYear('microbio_header.created_at', $periode[0])
                     ->whereMonth('microbio_header.created_at', $periode[1]);
            }
        }
