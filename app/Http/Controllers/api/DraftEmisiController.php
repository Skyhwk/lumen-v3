...
class DraftEmisiController extends Controller
{
        ...
        if ($request->category2 == 32 || $request->category2 == 31) {
            ...
            try {
                ...
                try {
                    ...
                    // dd($allDetail); // dump semua detail
                    ...
                    if ($header != null) {
                        ...
                        $job = new RenderLhp($header, $allDetail, 'downloadWSDraft', $groupedByPage);
                        $this->dispatch($job);

                        $job = new RenderLhp($header, $allDetail, 'downloadLHP', $groupedByPage);
                        $this->dispatch($job);

                        $job = new RenderLhp($header, $allDetail, 'downloadLHPFinal', $groupedByPage);
                        $this->dispatch($job);
    ...
    {
        try {
            $data = MetodeSampling::where('kategori', 'Emisi')->where('sub_kategori', 'like', '%' . $request->sub_kategori . '%')->get();
            return response()->json([
                'status' => true,
                'data' => $data
            ], 200);
