...
class LabDataController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($grade == 'STAFF'){
            $data = Ftc::join('master_karyawan','t_ftc.user_fd_lab','=','master_karyawan.id')
                ->select('master_karyawan.nama_lengkap','t_ftc.id','no_sample','ftc_fd_lab','user_fd_lab')
                ->where('user_fd_lab',$user->id)
                ->orderBy('ftc_fd_lab','DESC');
    ...
    public function store(Request $request){
        try {
            ...
            $data->ftc_fd_lab = Carbon::now()->format('Y-m-d H:i:s');
            $data->user_fd_lab = $this->user_id;
            $data->save();
    ...
    public function delete(Request $request){
        $data = Ftc::where('no_sample',$request->no_sample)->first();
        $data->ftc_fd_lab = null;
        $data->user_fd_lab = null;
        $data->save();
