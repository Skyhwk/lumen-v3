...
class WsFinalUdaraMikrobiologiUdaraController extends Controller
{
    ...
    {
        $data = OrderDetail::with(['detailMicrobiologi', 'udaraMicrobio', 'udaraSubKontrak'])
            ...
            function applyFormula($value, float $factor, $parameter)
            {
                ...
                if (!is_nan($MDL)) {
                    if (cekSpecialChar($value)) {
                        // Cek karakter spesial sebelum perhitungan
    ...
    public function saveData(Request $request)
    {
        ...
        if ($kategori_koreksi) {
            switch ($kategori_koreksi) {
                //AIR
                case '11':
                    $udara = LingkunganHeader::with('ws_value_linkungan')->where('no_sampel', $request->no_sampel)->where('is_active', 1)->first();
    ...
    {
        $data = MasterRegulasi::where('id_kategori', 4)->where('is_active', true)->get();
        return response()->json(
            [
                'data' => $data,
            ],
    ...
    {
        $data = DB::table('tabel_regulasi')->whereJsonContains('id_regulasi', (string) $request->id)->first();
        return response()->json(
            [
                'data' => $data,
            ],
    ...
    public function handleReject(Request $request)
    {
        DB::beginTransaction();
        try {
            MicrobioHeader::where('no_sampel', $request->no_sampel)->update([
                'is_active' => 0,
                'rejected_by' => $this->karyawan,
    ...
    public function handleApproveSelected(Request $request)
    {
        ...
        try {
            ...
            MicrobioHeader::whereIn('no_sampel', $request->no_sampel_list)->update([
                'lhps' => 1,
            ]);
