...
class LimsFdlSampahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = DataLapanganSampah::whereHas('detail', function ($query) use ($dbLims) {
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $data = DataLapanganSampah::find($request->id);
            if (!$data) {
            ...
            if (!empty($sampah)) {
                ...
                if ($status) {
                    ...
                    app(NotificationFdlService::class)->sendApproveNotification('Observasi Sampah', $data->no_sampel, $this->karyawan, $data->created_by);
                }
            }
    ...
    protected function autoBlock()
    {
        ...
        $data = DataLapanganSampah::where('is_blocked', 0)->where('created_at', '<=', $tgl)->update(['is_blocked' => 1, 'blocked_by' => 'System', 'blocked_at' => Carbon::now()->format('Y-m-d H:i:s')]);
    }
