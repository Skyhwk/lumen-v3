...
class LimsGravimetriController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Gravimetri::with('ws_value')
            ->join(DB::raw("{$limsDb}.order_detail as order_detail"),'order_detail.no_sampel','=','gravimetri.no_sampel')
            ...
            ->select('gravimetri.*', 'order_detail.tanggal_terima as od_tanggal_terima', 'order_detail.kategori_3 as od_kategori_3')
            ->orderByRaw("
                CASE 
        ...
        if ($request->filled('periode')) {
            ...
            if (count($periode) == 2) {
                $data->whereYear('gravimetri.created_at', $periode[0])
                     ->whereMonth('gravimetri.created_at', $periode[1]);
            }
        }
    ...
    public function approveData(Request $request)
    {
        DB::beginTransaction();
        try {
            $data = Gravimetri::where('id', $request->id)->where('is_active', true)->first();
            if ($data->is_approved == 1) {
                return response()->json([
                    'status' => false,
                    'message' => 'Data Gravimetri no sample ' . $data->no_sampel . ' sudah di approve'
                ], 401);
            }
            $data->is_approved = 1;
            ...
            return response()->json([
                'status' => true,
                'message' => 'Data gravimetri no sample ' . $data->no_sampel . ' berhasil di approve'
            ], 200);
    ...
    public function deleteData(Request $request)
    {
        DB::beginTransaction();
        try {
            $data = Gravimetri::where('id', $request->id)->first();
            ...
            $ws_value = WsValueAir::where('id_gravimetri', $request->id)->where('is_active', true)->first();
            if ($ws_value) {
            ...
            return response()->json([
                'status' => true,
                'success' => true,
                'message' => 'Data gravimetri no sample ' . $data->no_sampel . ' berhasil dihapus .!'
            ], 200);
