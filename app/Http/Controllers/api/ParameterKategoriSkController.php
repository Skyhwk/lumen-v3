...
class ParameterKategoriSkController extends Controller
{
    public function index()
    {
        DB::beginTransaction();
        try {
            $data = ParameterKategoriSk::where('is_active', true);
    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->id != '') {
                $parameter = ParameterKategoriSk::find($request->id);

                if ($parameter) {
                    $parameter->nama_kategori_asli = strtolower(str_replace(' ', '_', $request->nama_kategori));
                    $parameter->nama_kategori = $request->nama_kategori;
                    $parameter->parameters = json_encode($request->parameter, JSON_UNESCAPED_UNICODE);
                    $parameter->kategori = $request->kategori;
                    $parameter->sub_kategori = $request->sub_kategori;
                ...
                $parameter = new ParameterKategoriSk();
                $parameter->nama_kategori_asli = strtolower(str_replace(' ', '_', $request->nama_kategori));
                $parameter->nama_kategori = $request->nama_kategori;
                $parameter->parameters = json_encode($request->parameter, JSON_UNESCAPED_UNICODE);
                $parameter->kategori = $request->kategori;
    ...
    {
        if($request->id !=''){
            $data = ParameterKategoriSk::where('id', $request->id)->first();
            if($data){
