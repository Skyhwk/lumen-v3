...
class WsFinalUdaraController extends Controller
{
		...
		if ($request->kategori == 11) {
			...
			try {
				...
				if ($data) {
					...
					$urutan = $lapangan2->search($data2->no_sampel);
					$urutanDisplay = $urutan + 1;
					$data2['urutan'] = "{$urutanDisplay}/{$totLapangan}";
	...
	{
		try {
			if (in_array($request->kategori, $this->categoryLingkunganKerja) || in_array($request->kategori, $this->categoryLingkunganHidup)) {
		...
		if ($request->id) {
			...
			if (in_array($request->kategori, $this->categoryLingkunganKerja) || in_array($request->kategori, $this->categoryLingkunganHidup)) {
				if ($request->data_type == 'lingkungan') {
					...
					// dd($data);
					if ($data) {
						$cek = SinarUvHeader::where('id', $data->id)->first();
						$cek->lhps = 0;
						$cek->save();
						return response()->json([
							'message' => 'Data has ben Rejected',
				...
				$data = KebisinganHeader::where('parameter', $request->parameter)->where('lhps', 1)->where('no_sampel', $request->no_sampel)->first();
				// dd($data);
				if ($data) {
					$cek = KebisinganHeader::where('id', $data->id)->first();
					$cek->lhps = 0;
					$cek->save();
					return response()->json([
						'message' => 'Data has ben Rejected',
					...
					$dat = KebisinganHeader::where('id', $request->id)->first();
					$dat->lhps = 1;
					$dat->save();
					return response()->json([
						'message' => 'Data has ben Approved',
	...
	private function handleLingkungan($request, $no_sampel, $parameter, $hasil_c, $hasil_c1, $hasil_c2, $udara, $faktor_koreksi)
	{
		try {
			...
			if ($po) {
				...
				if ($lingkungan) {
					$valuews = WsValueLingkungan::where('lingkungan_header_id', $lingkungan->id)
						->where('is_active', 1)
						->first();
	...
	public function updateTindakan(Request $request)
	{
		...
		try {
			...
			if (
				...
				in_array($request->kategori, $this->categoryPencahayaan) ||
