...
class PlanktonController extends Controller
{

    public function index(Request $request)
    {
        // dd($request->all());
        $data = Colorimetri::with('ws_value')->where('parameter', 'Plankton')
        ...
        try {
            ...
            if ($data->is_approved == 1) {
                return response()->json([
                    'status' => false,
                    'message' => 'Data Plankton dengan no sample ' . $data->no_sampel . ' sudah di approve'
                ], 401);
            }
            $data->is_approved = 1;
            ...
            return response()->json([
                'status' => true,
                'message' => 'Data Plankton dengan no sample ' . $data->no_sampel . ' berhasil di approve'
            ], 200);
    ...
    public function rejectSampel(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json([
                'message' => 'Success Reject Data Plankton No Sampel ' . $request->no_sampel,
                'success' =>  true
            ], 200);
        } catch (\Exception $th) {
            DB::rollBack();
