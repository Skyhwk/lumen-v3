...
class PersiapanSampleController extends Controller
{
        ...
        try {
            ...
            foreach ($dataBotol as $botol) {
                ...
                if ($kategori == '1-Air' && !empty($botol['persiapan'])) {
                    ...
                    $cek = KonfigurasiPraSampling::whereIn('parameter', $botol['parameter'])->get();
                    ...
                    // hapus aja klo gaperlu =================================
                    // if ($cek->isEmpty()) {
                    //     foreach ($botol['parameter'] as $item) {
    ...
    {
        $psh = PersiapanSampelHeader::where('no_quotation', $request->no_quotation)
            ->where('no_order', $request->no_order)
            ->whereJsonContains('no_sampel', $request->no_sampel)
            ->first();

        if (!$psh)
            return response()->json(['message' => 'Sampel belum disiapkan'], 404);
    ...
    private function saveHeader($request)
    {
        // check existing psh
        // $existingPsh = PersiapanSampelHeader::where([
        //     'no_quotation' => $request->no_quotation,
        //     'periode' => $request->periode,
        //     'no_order' => $request->no_order,
        //     'tanggal_sampling' => $request->tanggal_sampling,
        // ])->first();
        $noSampel = array_keys($request->detail);

        $existingPsd = PersiapanSampelDetail::whereIn('no_sampel', $noSampel)->pluck('id_persiapan_sampel_header')->unique()->toArray();
        if (!empty($existingPsd) && count($existingPsd) == 1) {
            $unrelatedDetails = PersiapanSampelDetail::whereNotIn('no_sampel', $noSampel)->whereIn('id_persiapan_sampel_header', $existingPsd)->delete();
            // update psh
            $psh = PersiapanSampelHeader::whereIn('id', $existingPsd)->first();
        } else {
            // new psh
            $psh = new PersiapanSampelHeader();

            $documentNumber = $this->getDocumentNumber($request->no_order, $request->periode);

            $psh->no_document = $documentNumber;
    ...
    private function saveDetail($request, $psh)
    {
        if (!$request->detail)
            return false;
        ...
        foreach ($request->detail as $sampleNumber => $categories) {
            ...
            foreach ($categories as $category => &$params) {
                $od = $orderDetail->where('no_sampel', $sampleNumber)->first();
                $kategori = strtolower(explode("-", $od->kategori_2)[1]);
                $validCategory = $kategori === $category;

                if ($od && $validCategory) {
                    foreach ($params as $param => &$info) {
                        if ($kategori == "air") {
                            $decodedPersiapan = collect(json_decode($od->persiapan))->where('type_botol', $param)->first();
                        } else {
                            $decodedPersiapan = collect(json_decode($od->persiapan))->where('parameter', $param)->first();
                        }

                        $info['file'] = $decodedPersiapan->file;
                    }
                }
            }
    ...
    private function saveQrDocument($psh)
    {
        foreach (['persiapan_sampel', 'coding_sample', 'surat_tugas_pengambilan_sampel', 'berita_acara_sampling'] as $item) {
            ...
            $qr->data = json_encode([
                ...
                'tanggal_sampling' => Carbon::parse($psh->tanggal_sampling)->translatedFormat('F Y'),
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            $psd = $this->saveDetail($request, $psh);
            if (!$psd)
                throw new \Exception('Sampel tidak valid');
