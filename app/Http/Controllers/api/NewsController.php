...
class NewsController extends Controller
{
    ...
    {
        $data = News::with(['category'])->where('is_active', true)->orderBy('created_at', 'desc')->get();
    ...
    public function getNewsCatergoryApi(Request $request)
    {
        $data = News::with(['category'])->where('category_news_id', $request->id_category)->where('is_active', true)->orderBy('created_at', 'desc')->get();
        $data->map(function ($item) {
            $item->content = Repository::dir('news_content')->key(explode('.',$item->content)[0])->get();
            $imagePath = public_path('profile/news/' . $item->image);
            if (file_exists($imagePath)) {
    ...
    {
        $data = CategoryNews::where('is_active', true)->select('id', 'title')->get();
        return DataTables::of($data)->make(true);
    }
        ...
        try {
            ...
            if ($data) {
                ...
                $data->update([
                    ...
                    'category_news_id' => $request->category_id ?? $data->category_news_id,
                ...
                $data = News::create([
                    ...
                    'category_news_id' => $request->category_id,
                    'image' => $filename_img,
