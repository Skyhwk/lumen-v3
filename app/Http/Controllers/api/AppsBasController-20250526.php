...
class AppsBasController extends Controller
{
    ...
    //         // Generate sample numbers efficiently
    //         $noSample = collect($request->kategori)
    //             ->map(function($item) use ($request) {
    //                 $parts = explode(" - ", $item);
    //                 return $request->no_order . '/' . $parts[1];
    //             })
    //             ->toArray();

    //         // Single query to get sampling plan with validation
    //         $sp = SamplingPlan::where([
    //             ['id', '=', $infoSampling['id_sp']],
    //             ['quotation_id', '=', $infoSampling['id_request']],
    //             ['status_quotation', '=', $infoSampling['status_quotation']],
    //             ['is_active', '=', true]
    //         ])->first();
    ...
    //         // Optimized schedule query - get dates and sampler data in one go
    //         $jadwalData = Jadwal::select([
    //             'tanggal',
    //             'sampler',
    //             'kategori'
    //         ])
    ...
    //         $jadwal = $jadwalData->pluck('tanggal')->unique();
    //         $samplerJadwal = $jadwalData->map(function($item) {
    //             return (object)['sampler' => $item->sampler, 'kategori' => $item->kategori];
    //         });

    //         // Get order header and preparation header in parallel queries
    //         $orderH = OrderHeader::where([
    ...
    //         // Set detail_bas_documents
    //         $orderH->detail_bas_documents = ($persiapanHeader && !empty($persiapanHeader->detail_bas_documents)) 
    //             ? $persiapanHeader->detail_bas_documents 
    //             : json_encode([]);

    //         // Optimized order detail query with eager loading
    //         $orderD = OrderDetail::with(['codingSampling:id,jumlah_label', 'orderHeader:id,konsultan,no_order'])
    //             ->where([
    //                 ['id_order_header', '=', $orderH->id],
    //                 ['no_order', '=', $request->no_order],
    //                 ['is_active', '=', true]
    //             ])
    ...
    //         $data_sampling = [];
    //         $dat_param = [];
    //         $allNoSamples = $orderD->pluck('no_sampel')->toArray();

    //         // Bulk check for Air category status
    //         $airSamples = $orderD->where('kategori_2', '1-Air')->pluck('no_sampel');
    //         $completedAirSamples = [];
    //         if ($airSamples->isNotEmpty()) {
    //             $completedAirSamples = DataLapanganAir::whereIn('no_sampel', $airSamples)
    //                 ->pluck('no_sampel')
    //                 ->toArray();
    //         }
    ...
    //                 'konsultan' => $vv->orderHeader->konsultan ?? null,
    ...
    //                 'no_order' => $vv->orderHeader->no_order ?? null,
    //                 'no_document' => $request->no_document,
    //             ];

    //             // Determine status efficiently
    //             if ($vv->kategori_2 === "1-Air") {
    //                 $status[$vv->no_sampel] = in_array($vv->no_sampel, $completedAirSamples) ? 'selesai' : 'belum selesai';
    //             } else {
    //                 $status[$vv->no_sampel] = $this->getStatusSampling($vv);
    //             }

    //             // Get field data only if needed
    ...
    //                     if (!isset($hariTanggal[$vv->no_sampel]) || $dataLapangan->created_at > $hariTanggal[$vv->no_sampel]) {
    //                         $hariTanggal[$vv->no_sampel] = $dataLapangan->created_at;
    //                     }
    //                 }
    //             }

    //             if (!isset($hariTanggal[$vv->no_sampel])) {
    ...
    public function preview(Request $request)
    {
        ...
        try {
            ...
            $persiapanHeader = PersiapanSampelHeader::with('psDetail')->where([
                'no_order' => $request->no_order,
    ...
    //     if($parameter['parameter'] == 'PM 10 (24 Jam)' || $parameter['parameter'] == 'PM 2.5 (24 Jam)') {
    //         $model3 = $parameter['model3'];
    //     }

    //     $verified = $model::where('no_sampel', $sample_number);

    //     if ( $model == DetailLingkunganHidup::class || $model == DetailSenyawaVolatile::class || $model == DetailMicrobiologi::class || $model == DataLapanganDirectLain::class){
    ...
    //         if($model3 == null) {
    //             if($parameter['parameter'] == 'Suhu' || $parameter['parameter'] == 'Kelembaban' && $verified->count() < $parameter['requiredCount']) {

    //                 $paramName = $parameter['parameter'];
    //                 if ($paramName == 'Kelembaban') {
    //                     $paramName = 'Kelembapan';
    //                 }

    //                 $verified = $model::where('no_sampel', $sample_number)->whereNotNull($paramName)
    //                 ->first();
    //                 if($verified == null) {
    //                     $verified = $model2::where('no_sampel', $sample_number)->whereNotNull($paramName)
    //                     ->first();
    //                 }
    ...
    //             };
    //         } else {
    //             $verified = $model::where('no_sampel', $sample_number)->where('parameter', $parameter['parameter']);
    //             $data = $model::where('no_sampel', $sample_number)->where('parameter', $parameter['parameter'])->get();
    //             if($data->isEmpty()) {
    //                 $verified = $model2::where('no_sampel', $sample_number)->where('parameter', $parameter['parameter']);
    //                 $data = $model2::where('no_sampel', $sample_number)->where('parameter', $parameter['parameter'])->get();
    //                 if($data->isEmpty()) {
    //                     $verified = $model3::where('no_sampel', $sample_number)->where('parameter', $parameter['parameter']);
    //                 }
    //             }
    //         }
    ...
    //     ");
    // };
    // if($sample_number == 'ALUR012501/002') {
    ...
    private function getRequiredParameters()
    {
        ...
        return [
            ...
            [
                ...
                "category" => "5-Emisi",
                "model" => DataLapanganEmisiKendaraan::class,
                "model2" => DataLapanganEmisiCerobong::class
            ],
            [
                "parameter" => "Ergonomi",
                "requiredCount" => 1,
