...
class LimsTitrimetriController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Titrimetri::with('ws_value')
            ->join(DB::raw("{$limsDb}.order_detail as order_detail"),'order_detail.no_sampel','=','titrimetri.no_sampel')
            ...
            ->select('titrimetri.*', 'order_detail.tanggal_terima as od_tanggal_terima', 'order_detail.kategori_3 as od_kategori_3')
                ->orderByRaw("
                CASE 
        ...
        if ($request->filled('periode')) {
            ...
            if (count($periode) == 2) {
                $data->whereYear('titrimetri.created_at', $periode[0])
                     ->whereMonth('titrimetri.created_at', $periode[1]);
            }
        }
