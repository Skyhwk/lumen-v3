...
class LimsLhpKebisinganPersonalController extends Controller
{
    ...
    public function handleDownload(Request $request) {
        try {
            ...
            if ($noLhp) {
                $header = LhpsKebisinganPersonalHeader::where('no_lhp', $noLhp)->where('is_active', true)->first();
            } else {
                $header = LhpsKebisinganPersonalHeader::where('no_sampel', $request->no_sampel)->where('is_active', true)->first();
            }
    ...
    public function previewLhp(Request $request)
    {
        try {
            $noLhp = $request->no_lhp ?? $request->cfr;
            if ($noLhp) {
                $header = LhpsKebisinganHeader::where('no_lhp', $noLhp)->where('is_active', true)->first();
            } else {
            ...
            if ($header->file_qr == null) {
                ...
                $file_qr_path = $file_qr->insert('LHP_KEBISINGAN_PERSONAL', $header, $this->karyawan ?? 'System');
                if ($file_qr_path) {
