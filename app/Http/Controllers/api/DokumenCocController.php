...
class DokumenCocController extends Controller
{
    ...
    public function generateDokumenCoc(Request $request)
    {
        ...
        $historyAppRejectTqc = HistoryAppReject::where('no_lhp', $no_lhp)->where('menu', 'like', 'TQC%')->whereNotNull('approved_at')->first();
        ...
        $tglApproveTqc = optional($historyAppRejectTqc)->approved_at;
        ...
        $dokumenCoc->tgl_mulai_sampling = $orderDetail->min('tanggal_sampling');
        $dokumenCoc->tgl_selesai_sampling = $orderDetail->flatMap->any_data_lapangan->max(fn($item) => $item->created_at);
        $dokumenCoc->tgl_terima_lab = $tglTerimaMin;
        $dokumenCoc->tgl_mulai_analisa = $tglTerimaMin;
        $dokumenCoc->tgl_selesai_analisa = $tglApproveLapanganHeaderMax;
        $dokumenCoc->tgl_mulai_tcc = $tglApproveLapanganHeaderMax;
        $dokumenCoc->tgl_selesai_tcc = $tglApproveTqc;
        $dokumenCoc->tgl_mulai_drafting = $tglApproveTqc;
