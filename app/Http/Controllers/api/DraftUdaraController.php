...
class DraftUdaraController extends Controller
{
    // done if status = 2
    ...
    public function index(Request $request)
    {
        ...
        if ($kategori == null || $kategori == '') {
            ...
            $kategori = $subKategori->id . '-' . $subKategori->nama_sub_kategori;
        }
        ...
        if ($kategori == 27) {
            ...
            } else if ($request->parameter == 'Sinar') {
                $data = OrderDetail::with([
                    'lhps_getaran',
                    'lhps_kebisingan',
                    'lhps_ling',
    ...
    public function store(Request $request)
    {
        ...
        if (in_array($category, $categoryKebisingan)) {
            ...
            try {
                ...
                foreach ($request->noSampel as $key => $val) {
                    if (in_array("Getaran (LK) ST", $parameter) || in_array("Getaran (LK) TL", $parameter)) {
                        // dd($request->all());
            ...
            try {
                // Pencahayaan
                $header = LhpsPencahayaanHeader::where('no_lhp', $request->no_lhp)->where('no_order', $request->no_order)->where('id_kategori_3', $category)->where('is_active', true)->first();
                if ($header == null) {
            ...
            try {
                ...
                if (in_array("Medan Listrik", $orderDetailParameter) || in_array("Medan Magnit Statis", $orderDetailParameter) || in_array("Power Density", $orderDetailParameter)) {
                    // dd('Medan Listik, Medan Magnit Statis, Power Density');
                    $header = LhpMedanLmHeader::where('no_lhp', $request->no_lhp)->where('no_order', $request->no_order)->where('kategori_3', $request->kategori_3)->where('is_active', true)->first();

                    if ($header == null) {
                        $header = new LhpMedanLmHeader;
                        $header->create_by = $this->userid;
                        $header->ceate_at = DATE('Y-m-d H:i:s');
                    } else {
                        $history = $header->replicate();
                        ...
                        $history->updated_at = null;
                        $history->save();
                        $header->update_by = $this->userid;
                        $header->update_at = DATE('Y-m-d H:i:s');
                    }
                    ...
                    $detail = LhpMedanLmDetail::where('id_header', $header->id)->first();
                    if ($detail != null)
                        $history = $detail->replicate();
                    ...
                    $history->created_at = Carbon::now()->format('Y-m-d H:i:s');
                    $history->save();
                    $detai = LhpMedanLmDetail::where('id_header', $header->id)->delete();
                    foreach ($request->no_sample as $key => $val) {
                        $detail = new LhpMedanLmDetail;
                        $detail->id_header = $header->id;
                        $detail->no_sampel = $request->no_sample[$key];
                        $detail->lokasi_keterangan = $request->lokasi_keterangan[$key];
                        ...
                        $detail->hasil = $request->hasil[$key];
                ...
                DB::commit();
                return response()->json([
                    'message' => 'Data draft LHP Lingkungan no sampel ' . $request->no_sampel . ' berhasil disimpan',
                    'status' => true
                ], 201);
            } catch (\Exception $th) {
                DB::rollBack();
    ...
    public function handleDetailEdit(Request $request)
    {
        ...
        if (in_array($category, $categoryKebisingan)) {
            ...
            try {
                // Lingkungan Kerja
                // dd('Lingkungan Kerja', $request->all());
        ...
        try {
            ...
            // Lingkungan Kerja
                ...
                if ($orderDetail) {
                    ...
                    // Medan Listrik, Medan Magnit Statis, Power Density
                    if (in_array("Medan Listrik", $orderDetailParameter) || in_array("Medan Magnit Statis", $orderDetailParameter) || in_array("Power Density", $orderDetailParameter)) {

                        $lhps = LhpMedanLmHeader::where('no_lhp', $data->cfr)
                            ->where('no_order', $data->no_order)
                        ...
                        if ($lhps) {
                            ...
                            $oldDetails = LhpMedanLmDetail::where('id_header', $lhps->id)->get();
                            foreach ($oldDetails as $detail) {
                                $detailHistory = $detail->replicate();
