...
class CodingSampleController extends Controller
{
    ...
    public function preview(Request $request)
    {
        try {
            ...
            $psHeader = PersiapanSampelHeader::with('psDetail')->where([
                ...
                ->whereJsonContains('no_sampel', $noSampel)
                ->first();
            if ($psHeader) {
            ...
            foreach ($orderDetail as &$item) {
                $jumlahBotol = 0;
                $jumlahLabel = 0;
                // if($item->kategori_3 == 'Emisi Kendaraan (Bensin)'){
                //     $parameters = json_decode($item->parameters);
                //     foreach ($parameters as $category) {
                //         foreach ($category as $values) $jumlahBotol += 0;
                //     }
                // }
                    ...
                    if ($item->no_sampel == $psd->no_sampel) {
                        ...
                        foreach ($parameters as $category) {
                            foreach ($category as $key => $values) {
                                ...
                                // kalo air dikali 2 krna butuh 2 label (di botol sm di tutupnya)
                                // $jumlahLabel += $category == 'air' ? (int) $values->disiapkan * 2 : (int) $values->disiapkan;
                                $jumlahLabel = $jumlahLabel + ((int) $values->disiapkan * 2);
                                // dump($key,((int) $values->disiapkan * 2));
                            }
                        }
            ...
            switch ($request->type_file) {
                ...
                case 'qrcode':
                    return response()->json($this->cetakQRCodePDF($psDetail,$psHeader->tanggal_sampling), 200);
                case 'label':
                    $isLabeled = false;
                    foreach ($psDetail as $psd) {
    ...
    //         // $orderDetail = $quotation->order->orderDetail()->whereIn('no_sampel', $noSampel);
    //         $orderDetail = $quotation->order->orderDetail()
    ...
    //                 }
    //             }
    //             // dump($item->no_sampel, $jumlahLabel);
    ...
    //                 return response()->json($this->cetakPDF($orderDetail, $bsDocument, $psHeader), 200);
    //             case 'qrcode':
    //                 /* perisapan form Order Detail */
    //                 $queryPersiapanOrderDetail = $quotation->order->orderDetail()
    //                     ->where('tanggal_sampling', $request->tanggal_sampling)
    //                     ->whereIn('no_sampel', $noSampel)
    //                     ->whereNotNull('persiapan');
    //                 $perisapanOrderDetail = $queryPersiapanOrderDetail->get();
    //                 if ($perisapanOrderDetail->count() == count($noSampel)) {
    //                     $psDetail = $perisapanOrderDetail;
    //                     $isGenerateQR = false;
    //                 } else {
    //                     $isGenerateQR = true;
    //                 }
    //                 return response()->json($this->cetakQRCodePDF($psDetail, $isGenerateQR), 200);
    //             case 'label':
    //                 $isLabeled = false;
    //                 foreach ($psDetail as $psd) {
    //                     $hasZero = collect(json_decode($psd->parameters, true))
    ...
    /* private function cetakQRCodePDF($psDetail)
    {
        $pdf = new Mpdf([
            'mode' => 'utf-8',
            'format' => [50, 15],
                    ...
                    for ($i = 0; $i < $qty->disiapkan; $i++) {
                        ...
                        $pdf->WriteHTML('
                                        <th style="padding: ' . $padding . ';">
                                            <table>
                                                <tr>
                                                    <td style="text-align: left;"><img src="' . public_path() . '/qrcode/sample/' . $qrCode . '"></td>
    ...
    private function cetakQRCodePDF($psDetail, $tanggal = null)
    {
        try {
            $pdf = new Mpdf([
                'mode' => 'utf-8',
            ...
            // Pastikan direktori cs ada
            $csDir = public_path() . '/cs/';
            if (!is_dir($csDir)) {
            ...
            return [
                'status' => true,
                'data' => $filename,
                'message' => 'QR Code PDF berhasil dibuat'
            ];

        } catch (\Exception $th) {
            \Log::error("Error in cetakQRCodePDF: " . $th->getMessage(), [
                'line' => $th->getLine(),
                'file' => $th->getFile(),
    ...
    /* private function cetakQRCodePDF($psDetail,$tanggal = null)
    {
        try {
            ...
            foreach ($psDetail as $psd) {
                ...
                if ($kodingSampling != null) {
                    $orderPesiapanUpdate = json_decode($kodingSampling->persiapan, true) ?? [];
                    ...
                    foreach ($parameters as $key => $categories) {
                        ...
                        foreach ($categories as $parameter => $qty) {
                            ...
                            if ($qty->file != null) {
                                ...
                                $qty->file = $persiapan['file']; // penting
                            }
                            $this->generateQR($persiapan['koding'],'/barcode/penjerap');
                            $orderPesiapanUpdate[] = $persiapan;
                    ...
                    $kodingSampling->persiapan = json_encode($orderPesiapanUpdate); //update order detail colum perisapan
                    $kodingSampling->save();
    
                    // simpan kembali parameters hasil update file
                    $psd->parameters = json_encode($parameters); //update parameter di persiapan detail
                    $psd->save();
                }
            }
