...
class FdlObservasiMinyakController extends Controller
{
    public function index(Request $request)
    {
        $this->autoBlock();
        $data = DataLapanganLapisanMinyak::with('detail')->orderBy('id', 'desc');
    ...
    public function updateNoSampel(Request $request)
    {
        if (isset($request->id) && $request->id != null) {
            DB::beginTransaction();
            try {
                $data = DataLapanganLapisanMinyak::where('no_sampel', $request->no_sampel_lama)->whereNull('no_sampel_lama')->get();
    ...
    public function approve(Request $request)
    {
        ...
        try {
            ...
            $data = DataLapanganLapisanMinyak::find($request->id);
            if (!$data) {
            ...
            $parameterArray = json_decode($order_detail->parameter);
            $minyak = null;

            foreach ($parameterArray as $item) {
                if (stripos($item, 'Lapisan Minyak') !== false) {
                    $parts = explode(';', $item);
                    $minyak = $parts[1] ?? null;
                    break;
                }
            ...
            if (!empty($minyak)) {
                ...
                $nilaiArah = array_map(function ($arah) {
                    ...
                    if ($arah === 'nihil' || $arah === 'tidak dapat diukur') {
                        return 0;
                    }
                ...
                if ($status) {
                    ...
                    app(NotificationFdlService::class)->sendApproveNotification('Observasi Lapisan Minyak', $data->no_sampel, $this->karyawan, $data->created_by);
                }
            }            
    ...
    protected function autoBlock()
    {
        ...
        $data = DataLapanganLapisanMinyak::where('is_blocked', 0)->where('created_at', '<=', $tgl)->update(['is_blocked' => 1, 'blocked_by' => 'System', 'blocked_at' => Carbon::now()->format('Y-m-d H:i:s')]);
    }
    ...
    public function rejectData(Request $request)
    {
        if (isset($request->id) && $request->id != null) {
            ...
            app(NotificationFdlService::class)->sendRejectNotification("Observasi Lapisan Minyak", $request->no_sampel, $request->reason, $this->karyawan, $data->created_by);
