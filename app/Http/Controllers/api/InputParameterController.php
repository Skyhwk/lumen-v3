...
class InputParameterController extends Controller
{
	...
	// 				// Ambil parameter dari 3 model berdasarkan no_sampel
	// 				$cekDurasi = collect()
	// 					->merge(
	// 						DetailLingkunganHidup::where('no_sampel', $request->no_sample)
	// 							->pluck('parameter')
	// 					)
	// 					->merge(
	// 						DetailLingkunganKerja::where('no_sampel', $request->no_sample)
	// 							->pluck('parameter')
	// 					)
	// 					->merge(
	// 						DetailSenyawaVolatile::where('no_sampel', $request->no_sample)
	// 							->pluck('parameter')
	// 					);
	// 				// Tentukan parameter Pb berdasarkan TSP
	// 				if ($cekDurasi->contains('TSP (24 Jam)')) {
	// 					$parame = 'Pb (24 Jam)';
	// 				} elseif ($cekDurasi->contains('TSP (8 Jam)')) {
	// 					$parame = 'Pb (8 Jam)';
	// 				} else {
	// 					$parame = 'Pb';
	// 				}
	...
	public function HelperLingkungan($request, $stp, $datlapanganh, $datlapangank, $datlapanganV)
	{
		...
		if ($wsling) {
			...
			if ($datlapanganh != null || $datlapangank != null || $datlapanganV != null) {
				...
				// Query pakai $parame_query bukan $parame
				$lingHidup    = DetailLingkunganHidup::where('no_sampel', $request->no_sample)->where('parameter', $parame_query)->get();
				$lingKerja    = DetailLingkunganKerja::where('no_sampel', $request->no_sample)->where('parameter', $parame_query)->get();
				$lingVolatile = DetailSenyawaVolatile::where('no_sampel', $request->no_sample)->where('parameter', $parame_query)->get();
				...
				if (!$lingHidup->isEmpty() || !$lingKerja->isEmpty() || !$lingVolatile->isEmpty()) {
					...
					try {
						...
						$datot = ($datapangan != '') ? count($datapangan) : '';
						...
						if ($datot > 0 || $datot != '') {
							...
							if ($isO3) {
								...
								// ✅ Cek apakah masuk kondisi 24 Jam
								$is24Jam = in_array($request->parameter, [
									'Pb (24 Jam)', 'PM 2.5 (24 Jam)', 'PM 10 (24 Jam)', 'TSP (24 Jam)'
								]) || $data_parameter->id == 306
								|| ($parame == 'Pb' && $pb_tipe_tsp == '24 Jam');
			...
			$data_parsing = (object) $request->all();
			$data_parsing->use_absorbansi = false;
			$data_parsing->tipe_data      = $tipe_data;
			...
			try {
				...
				if (in_array($data_parameter->id, $saveShift) || $request->id_stp == 13) {
					...
					} elseif (is_array($request->ks) && is_array($request->kb)) {
						$data_shift = array_map(function ($sample, $blanko) {
							return (object)[
								"sample" => number_format($sample, 4),
								"blanko" => number_format($blanko, 4)
