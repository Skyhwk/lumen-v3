...
class LimsIcpUdaraController extends Controller
{
    // 16-06-2026
    public function index(Request $request){
        $limsDb = \Illuminate\Support\Facades\DB::connection('lims')->getDatabaseName();

        $data = LingkunganHeader::with('ws_udara', 'ws_value')
            ->join(\Illuminate\Support\Facades\DB::raw("{$limsDb}.order_detail as order_detail"),'order_detail.no_sampel','=','lingkungan_header.no_sampel')
            ->where('lingkungan_header.is_active', true)
            ->where('is_approved', $request->approve)
            ->where('template_stp', $request->template_stp)
            ->select('lingkungan_header.*', 'order_detail.tanggal_terima as od_tanggal_terima', 'order_detail.kategori_3 as od_kategori_3', 'order_detail.tanggal_sampling as os_tanggal_sampling')
            ->orderByRaw("
                CASE 
        ...
        if ($request->filled('periode')) {
            ...
            if (count($periode) == 2) {
                $data->whereYear('lingkungan_header.created_at', $periode[0])
        ...
        return Datatables::of($data)
            ...
            ->addColumn('type_fdl', function ($item) {
                if($item->od_kategori_3 == "27-Udara Lingkungan Kerja")
                    return "ulk";
                else {
                    return "ambient";
                }
