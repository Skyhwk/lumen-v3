...
//             'no_sampel'       => $request->noSampel ?: null,
//             'id_kategori_2'   => $request->kategori_2 ? explode('-', $request->kategori_2)[0] : null,
//             'id_kategori_3'   => $category ?: null,
//             'no_qt'           => $request->no_penawaran ?: null,
//             'parameter_uji'   => json_encode($parameter),
...
//             'metode_sampling' => $request->metode_sampling ?: null,
//             'tanggal_sampling'=> $request->tanggal_tugas ?: null,
//             'periode_analisa' => $request->periode_analisa ?: null,
//             'nama_karyawan'   => 'Kharina Waty',
//             'jabatan_karyawan'=> 'Technical Control Manager',
...
//         // === Backup detail lama ===
//         foreach (LhpsGetaranDetail::where('id_header', $header->id)->get() as $detail) {
...
//         // Helper kecil untuk cleaning array
//         $clean = fn($arr) => array_combine(
//             array_map(fn($k) => trim($k, " '\""), array_keys($arr ?? [])),
//             array_values($arr ?? [])
//         );
...
//                     'id_header'   => $header->id,
//                     'no_sampel'   => $clean($request->no_sampel)[$val] ?? null,
//                     'keterangan'  => $clean($request->keterangan_detail)[$val] ?? null,
//                     'param'       => $clean($request->param)[$val] ?? null,
//                     'sumber_get'  => $clean($request->sumber_get)[$val] ?? null,
//                     'w_paparan'   => $clean($request->w_paparan)[$val] ?? null,
//                     'hasil'       => $clean($request->hasil)[$val] ?? null,
//                     'tipe_getaran'=> $clean($request->tipe_getaran)[$val] ?? null,
//                     'nab'         => $clean($request->nab)[$val] ?? null,
//                 ]);
//                 $detail->save();

//             } elseif (in_array("Getaran (LK) TL", $parameter)) {
//                 $detail = new LhpsGetaranDetail([
//                     'id_header'   => $header->id,
//                     'no_sampel'   => $clean($request->no_sampel)[$val] ?? null,
//                     'param'       => $clean($request->param)[$val] ?? null,
...
//                     'no_sampel'   => $val,
//                     'keterangan'  => $clean($request->keterangan_detail)[$val] ?? null,
//                     'percepatan'  => $clean($request->percepatan)[$val] ?? null,
//                     'kecepatan'   => $clean($request->kecepatan)[$val] ?? null,
//                     'tipe_getaran'=> $clean($request->tipe_getaran)[$val] ?? null,
//                 ]);
//                 $detail->save();
//             }
...
//                             'param'     => $request->custom_param[$page][$sampel] ?? null,
...
//             $view = in_array($header->sub_kategori, ["Getaran (Lengan & Tangan)", "Getaran (Seluruh Tubuh)"])
//                 ? 'DraftGetaranPersonal'
//                 : 'DraftGetaran';
