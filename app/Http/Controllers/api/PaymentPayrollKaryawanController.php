...
class PaymentPayrollKaryawanController extends Controller
{
    public function index(Request $request){
        try {
            ...
            ->where(function ($q) {
                $q->where('payroll_header.status', '!=', 'TRANSFER')
                  ->orWhereNull('payroll_header.status');
            })
    ...
    public function approvePayroll(Request $request){
        ...
        try {
            ...
            Payroll::where('payroll_header_id', $request->id)->where('is_actie', true)
                ->update([
                    'status' => 'PAID',
                ]);
    ...
    public function exportPerbankanMyBank(Request $request){
        try {
            ...
            foreach ($data as $k => $value) {
                ...
                $sheet->setCellValue('L' . $row, $value->take_home_pay);
                $sheet->setCellValue('M' . $row, 'Y');
                $sheet->setCellValue('N' . $row, 'IDR');
                $sheet->setCellValue('O' . $row, '2782000269');
