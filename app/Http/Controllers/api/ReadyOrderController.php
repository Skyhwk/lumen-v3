...
class ReadyOrderController extends Controller
{
                    ...
                    if ($value->kategori_1 == '1-Air') {
                        ...
                        if (count($value->parameter) <= 2) {
                            if ($kategori != $value->kategori_2 || json_encode($regulasi) != json_encode($value->regulasi) || in_array('Ergonomi', $parameterNames)) {
                                ...
                                if (
                                    ($kategori == $value->kategori_2 && json_encode($regulasi) == json_encode($value->regulasi) && count($parameter) > 2) ||
    ...
    //         // Ambil semua no_sampel dari order_detail
    ...
    //                         $orderDetail->tanggal_sampling = $cek_jadwal->tanggal;
    //                     $orderDetail->updated_at = Carbon::now()->format('Y-m-d H:i:s');
    //                     $orderDetail->save();
    //                 }

    //                 $jsonSampelList[] = $noSampel;

    //                 $sampelInfo[$noSampel] = [
    //                     'kategori_1' => $dps->kategori_1 ?? '-',
    //                     'kategori_2' => $dps->kategori_2 ?? '-',
    //                     'kode_titik' => $noUrutSampel,
    //                     'nama_titik' => $namaTitik ?? '-',
    //                     'no_sampel' => $noSampel,
    //                     'parameter' => $dps->parameter ?? [],
    //                     'regulasi' => $dps->regulasi ?? [],
    //                     'volume' => $dps->volume ?? 0,
    //                     'status_sampling' => $dataQuotation->status_sampling ?? 'S',
    //                 ];
    //             }
    //         }
    //         // dd($jsonSampelList, $sampelInfo, $orderDetails);
    //         $penambahan = array_diff($jsonSampelList, $orderDetails);
    //         $pengurangan = array_diff($orderDetails, $jsonSampelList);

    //         $dataPenambahan = array_map(function ($no_sampel) use ($sampelInfo) {
    //             return (object) ($sampelInfo[$no_sampel] ?? ['no_sampel' => $no_sampel]);
    //         }, $penambahan);

    //         $pengurang = array_values($pengurangan);
    //         $dataPenambahan = array_values($dataPenambahan);

    //         if ($pengurang != null) {
    //             $data = OrderDetail::where('no_order', $no_order)
    //                 ->whereIn('no_sampel', $pengurang)
    //                 ->update(['is_active' => 0]);

    //             Ftc::whereIn('no_sample', $pengurang)->update(['is_active' => 0]);
    //             FtcT::whereIn('no_sample', $pengurang)->update(['is_active' => 0]);
    //         }

    //         $n = 0;
    //         $no_urut_cfr = 0;
    //         if ($dataPenambahan != null) {
    //             // Add data
    ...
    //             $regulasi = [];
    //             $parameter = [];
    //             foreach ($dataPenambahan as $key => $value) {
    //                 // dd($value->parameter);
    //                 $no_sample = $value->no_sampel;
    //                 /*
    //                  * Disini bagian pembuatan no sample dan no cfr/lhp
    //                  * Jika jumlah parameter kurang dari 2 maka akan di cek apakah kategori sama atau tidak
    //                  * Jika kategori sama maka no akan di increment
    ...
    //                 $penamaan_titik = $value->nama_titik;
    ...
    //         // dd(json_decode($dps_details[0]['data_pendukung_sampling'], true));
    //         $sampel_detail_baru = [];
    //         $detail_baru = [];
    //         foreach ($dps_details as $dps) {
    //             $status_sampling = $dps["status_sampling"];
