        ...
        try {
            ...
            if ($request->has('no_document')) {
                if ($request->no_document != null || $request->no_document != '') {
                    ...
                    $sp = DB::table('sampling_plan')->where('id', $jadwal->sample_id)->where('active', 0)->first();
    ...
    public function cetakPDFSTPS(Request $request)
    {
        try {
            ...
            if ($tipe_penawaran == 'QTC') {
                ...
                $dataQuotation = $dataPenawaran->detail()
                    ->where('periode_kontrak', $request->periode)
                    ->select('data_pendukung_sampling')
                    ->first();

                $dataPendukung = [];

                if ($dataQuotation && $dataQuotation->data_pendukung_sampling) {
                    $decoded = json_decode($dataQuotation->data_pendukung_sampling, true); // pakai true agar jadi array
                    $periodeKey = array_key_first($decoded); // misalnya "12"
                    $dataPendukung = $decoded[$periodeKey]['data_sampling'] ?? [];
                }
                ...
                $dataOrder = $dataPenawaran->order;
                $dataPendukung = json_decode($dataPenawaran->data_pendukung_sampling, true);
            ...
            foreach ($dataPendukung as $key => $a) {
                if (is_array($a))
                    $a = (object) $a;
            ...
            $fileName = str_replace('/', '_', $no_document) . '.pdf';
