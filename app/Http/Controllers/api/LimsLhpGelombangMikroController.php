...
class LimsLhpGelombangMikroController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('month_year') && !empty($request->month_year)) {
            ...
            if (count($parts) === 2) {
                $year = $parts[0];
                $month = $parts[1];
                $matchingIds = \App\Models\LhpsMedanLMHeader::whereYear('tanggal_lhp', $year)
    ...
    public function previewLhp(Request $request)
    {
        try {
            ...
            $groupedByPage = collect(LhpsMedanLMDetail::where('id_header', $header->id)->get())
                ->groupBy('page')
                ->toArray();
