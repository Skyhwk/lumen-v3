    ...
    public function index(Request $request)
    {
       ...
       try {
            $periode_awal = Carbon::parse($request->periode_awal)
                ->startOfMonth() // Opsional tapi disarankan (untuk keamanan)
                ->format('Y-m-d'); 

            // Periode Akhir (Otomatis tanggal terakhir bulan itu)
            $periode_akhir = Carbon::parse($request->periode_akhir)
                ->endOfMonth()   // Lakukan ini DULU sebelum format
                ->format('Y-m-d'); // Baru ubah ke string
