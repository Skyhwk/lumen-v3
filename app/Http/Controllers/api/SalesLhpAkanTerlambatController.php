...
class SalesLhpAkanTerlambatController extends Controller
{
    private function getWorkDayThreshold(int $days)
    {
        $workDay = Carbon::now()->subWeekdays($days);

        $liburPerusahaan = LiburPerusahaan::where('tanggal', '>=', $workDay)
            ->where('tanggal', '<=', Carbon::now())
            ->get();

        return $workDay->copy()->subWeekdays($liburPerusahaan->count());
    }
    ...
    public function index(Request $request)
    {
        ...
        $workDay8 = $this->getWorkDayThreshold(8);
        $workDay10 = $this->getWorkDayThreshold(10);
        // Model::whereBetween('created_at', [$minDate, $maxDate])->get();
        ...
        $rekapOrder = DB::table('order_detail')
            ...
            ->where('order_detail.tanggal_sampling', '<=', $workDay8)
            ->where('order_detail.tanggal_sampling', '>', $workDay10)
        ...
        */
        ...
        return DataTables::of($rekapOrder)
            ->addColumn('total_hari', function ($data) {
