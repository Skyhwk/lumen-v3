...
class StpsController extends Controller
{
    public function index(Request $request)
    {
       ...
       try {
            ...
            ->where('is_active', true)
            ->where('is_downloaded_stps',false)
            ->where('is_printed_stps',false)
                ...
                foreach ($targetPlan->jadwal as $schedule) {
                    ...
                    if (isset($groupedData[$key])) {
                        ...
                        $groupedData[$key] = [
                            ...
                            'is_revisi'          => $orderHeader->is_revisi,
                            'nama_cabang'        => $namaCabang,
                            'is_downloaded'      => (int)$statusRow['is_downloaded_stps'], // 1 jika sudah download, 0 jika belum
                            'is_printed'      => (int)$statusRow['is_printed_stps'], // 1 jika sudah download, 0 jika belum
                        ];
                    }
                }
            }
