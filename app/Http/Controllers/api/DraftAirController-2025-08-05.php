...
class DraftAirController extends Controller
{
    ...
    public function handleDatadetail(Request $request)
    {
        try {
            ...
            if ($cek_lhp) {
                $cek_lhp->methode_sampling = json_decode($cek_lhp->methode_sampling) ?? [];
                $data_entry = array();
                $data_custom = array();
                $cek_regulasi = array();
                ...
                foreach ($mainData as $key => $value) {
                    $masterBaku = MasterBakumutu::where('id_regulasi', $request->regulasi)->where('parameter', $value['name'])->first();
                    // dd($value, $request->regulasi, $masterBaku);
                    if (isset($masterBaku)) {
                        $mainData[$key]['baku_mutu'][0] .= " **";
                    }
                }
    ...
    private function formatEntry($val, $regulasiId, &$methodsUsed = [], $mainData = [])
    {
        $param = $val->master_parameter;

        if ($val->parameter == 'pH (NA)') {
            $entry = [
                'id' => $val->id,
                'name' => $val->parameter,
