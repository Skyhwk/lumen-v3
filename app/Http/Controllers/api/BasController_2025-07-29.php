...
class BasController extends Controller
{
    ...
    // {
    //     try {
    //         $periode_awal = new DateTime($request->periode_awal);
    //         $periode_akhir = new DateTime($request->periode_akhir);
    //         $interval = $periode_awal->diff($periode_akhir);
    //         if ($interval->days > 62) return response()->json(['message' => 'Periode tidak boleh lebih dari 2 bulan'], 403);
    ...
    //             if (empty($item['order_header']) || empty($item['order_header']['sampling_plan'])) return $carry;

    //             $samplingPlan = $item['order_header']['sampling_plan'];
    ...
    public function preview(Request $request)
    {
        ...
        try {
            ...
            $psHeader = PersiapanSampelHeader::with('psDetail')->where([
                ...
                ->whereJsonContains('no_sampel', $noSample)
                ->first();
