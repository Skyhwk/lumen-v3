...
class RekapQuotationController extends Controller
{
    ...
    public function reOrderNonKontrak($dataQuotation, $no_order, $dataJadwal, $data_lama)
    {
        ...
        try {
            ...
            if ($perubahan_data != null) {
                foreach ($perubahan_data as $key => $value) {
                    foreach ($value as $k => $v) {
                        ...
                        // dd(json_encode($data_qt_lama->regulasi));
            ...
            $workerOperation->index($data, $data_to_log, GetAtasan::where('user_id', 599)->get()->pluck('email')->toArray());
    ...
    public function reOrderKontrak($dataQuotation, $no_order, $dataJadwal, $data_lama)
    {
        ...
        try {
            ...
            $workerOperation = new WorkerOperation();
            $workerOperation->index($updateHeader, $data_to_log, GetAtasan::where('user_id', 599)->get()->pluck('email')->toArray());
