...
class NectonController extends Controller
{
    ...
    {
        // dd($request->all());
        $data = Colorimetri::with('ws_value')->where('parameter', 'Necton')
        ...
        try {
            ...
            if ($data->is_approved == 1) {
                return response()->json([
                    'status' => false,
                    'message' => 'Data Necton dengan no sample ' . $data->no_sampel . ' sudah di approve'
                ], 401);
            }
            $data->is_approved = 1;
            ...
            return response()->json([
                'status' => true,
                'message' => 'Data Necton dengan no sample ' . $data->no_sampel . ' berhasil di approve'
            ], 200);
    ...
    public function rejectSampel(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json([
                'message' => 'Success Reject Data Necton Dengan No Sampel ' . $request->no_sampel,
                'success' => true
            ], 200);
        } catch (\Exception $th) {
            DB::rollBack();
