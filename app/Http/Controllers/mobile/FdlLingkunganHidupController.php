...
class FdlLingkunganHidupController extends Controller
{
    ...
    public function getShift(Request $request)
    {
        try {
            ...
            // Parameter dengan syarat jumlah data (durasi) yang belum genap threshold-nya
            $parNonSes = array();
            foreach ($param as $value) {
                $p = DetailLingkunganHidup::where('no_sampel', $request->no_sample)
                    ->where('parameter', $value->parameter)->get();
                ...
                if (str_contains($value->parameter, 'Dustfall')) {
                    ...
                    if (str_contains($value->parameter, 'PM') || str_contains($value->parameter, 'TSP')) {
                        if ($li[0] == '24 Jam') {
                            $shift = 25;
                        } else if ($li[0] == '8 Jam') {
                            $shift = 8;
            ...
            // Parameter yang belum ada record-nya sesuai kriteria shift (Sesaat/L1 utk L1, shift saat ini utk selainnya)
            $filtered_param = array_values(array_diff($nilai_param2, $lh_parameter));
    
            // Gabungkan: parameter durasi yang belum genap + parameter yang belum diinput sama sekali
        ...
        try {
            ...
            if(isset($request->param) && $request->param != null){
                ...
                foreach($filteredParameters as $a){
                    ...
                    if ($request->statFoto == 'adaFoto') {
                        if ($request->foto_lokasi_sampel != '') $fdlvalue->foto_lokasi_sampel = , 'dokumentasi/sampling'($request->foto_lokasi_sampel, 1, $this->user_id, 'dokumentasi/sampling');
                        if ($request->foto_alat != '') $fdlvalue->foto_kondisi_sampel = ImageConvertService::fromBase64($request->foto_alat, 2, $this->user_id, 'dokumentasi/sampling');
                        if ($request->foto_lain != '') $fdlvalue->foto_lain = ImageConvertService::fromBase64($request->foto_lain, 3, $this->user_id, 'dokumentasi/sampling');
                    } else {
                        if ($request->foto_lokasi_sampel != '') $fdlvalue->foto_lokasi_sampel = ImageConvertService::fromBase64($request->foto_lokasi_sampel, 1, $this->user_id, 'dokumentasi/sampling');
