...
class FlowMeterController extends Controller
{
    ...
    {
        $kodeList = $request->kode;

        if (!is_array($kodeList) || empty($kodeList)) {
            return response()->json([
                'message' => 'Parameter kode harus berupa array dan tidak boleh kosong'
            ], 422);
        }

        $devices = DeviceIntilab::whereIn('kode', $kodeList)->get();

        if ($devices->isEmpty()) {
            return response()->json([
                'message' => 'Tidak ada device yang ditemukan'
            ], 404);
        }

        $devices->makeHidden(['created_at', 'updated_at', 'created_by', 'updated_by']);
    ...
    public function getDeviceRunning(Request $request){
        $devices = DeviceIntilabRunning::where('is_active', true)->where('type', 'Flow Meter')->where('start_by', $this->karyawan)->get();
        ...
        try {
            ...
            foreach ($dataPerJam as $jam => $items) {
                ...
                if ($jumlah > 0) {
                    ...
                    $tempK = $avg_temp + 273;
                    $hasil = $avg_flow * pow((298 * $avg_pressure) / ($tempK * 760), 0.5);
