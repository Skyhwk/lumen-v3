...
class ScanBotolController extends Controller
{
	...
	public function getScanData(Request $request)
	{
		...
		if (isset($datachek[1])) {
			...
			// dd($data);
			$scan = ScanBotol::where('no_sampel', $data->no_sampel)->first();
		...
		return response()->json(['data' => $dataDisplay, 'data_scan' => $scan, 'no_sampel' => $data->no_sampel], 200);
	}
	...
	public function store(Request $request)
	{
		...
		// Set status
		$data->status = $this->allDisiapkanMatch($parameters, $inputData) ? 'selesai' : 'belum selesai';
