		...
		if ($request->qr != null) {
			...
			if ($cek) {
				if ($cek->type_document != 'signature') {
					...
					// dd($data);
					if (isset($data->Nomor_LHP) || in_array($cek->type_document, ['berita_acara_sampling', 'surat_tugas_pengambilan_sampel', 'coding_sample', 'persiapan_sampel', 'permintaan_dokumentasi_sampling', 'invoice','e_certificate_webinar', 'skppa', 'coc', 'LHP_SAR', 'skhp_sar', 'stps', 'PURCHASE_ORDER', 'kebijakan_perusahaan'])) {
						$array = (array) $data;
					} else {
						$array = [
