<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use Carbon\Carbon;

use App\Models\SummaryInvoice;
use App\Models\PointEarning;
use App\Models\DailyQsd;
use DB;

class CalculatePoinCustomer extends Command
{
    protected $signature = 'calculate:poin-customer';
    protected $description = 'Calculate poin customer';

    protected $basicPoint = 10000; // 10rb untuk 1 poin

    public function handle()
    {
        $startTime = microtime(true);

        $this->output->newLine();
        $this->info("  START CALCULATING POIN CUSTOMER  ");
        $this->info("  Time: " . Carbon::now()->toDateTimeString());
        // =======================START CALCULATING POIN CUSTOMER================================
        // $data = DailyQsd::where('is_lunas', 1)->where('is_point_calculated', 0)
        // ->orderBy('no_invoice')

        $data = SummaryInvoice::whereIn('status_lunas', ['Lunas', 'Kelebihan Pembayaran'])
        ->whereNotExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('point_earnings')
                ->whereColumn('point_earnings.source_id', 'summary_invoice.no_invoice')
                ->where('point_earnings.source_type', 'summary_invoice');
        })
        ->whereDate('tgl_pelunasan', '>=', '2025-01-01')
        ->orderBy('no_invoice')
        ->chunk(500, function ($rows) {

            foreach ($rows as $item) {
        
                try {
        
                    $exists = PointEarning::where('source_type', 'summary_invoice')
                        ->where('source_id', $item->no_invoice)
                        ->exists();
        
                    if ($exists) {
                        $this->error("  Point earning already exists: " . $item->no_invoice);
                        continue;
                    }
        
                    $poin = intval(floor($item->nilai_pelunasan / $this->basicPoint));
        
                    if ($poin <= 0) {
                        $this->error("  Poin is less than 0: " . $item->no_invoice);
                        continue;
                    }
        
                    $earnedAt = Carbon::parse($item->tgl_pelunasan);

                    if ($earnedAt->lte(Carbon::parse('2025-01-01'))){
                        $this->error("Tanggal pembayaran is less than 2025-01-01: " . $item->pelanggan_id . " - Total Poin: " . $poin . " - No Invoice: " . $item->no_invoice);
                        continue;
                    }
        
                    PointEarning::firstOrCreate(
                        [
                            'source_type' => 'summary_invoice',
                            'source_id'   => $item->no_invoice,
                        ],
                        [
                            'customer_id'       => $item->pelanggan_id,
                            'points'            => $poin,
                            'claimed_points'    => 0,
                            'expired_points'    => 0,
                            'earned_at'         => $earnedAt,
                            'claim_expired_at'  => $earnedAt->copy()->addYear(),
                            'tier_expired_at'   => $earnedAt->copy()->addYears(2),
                        ]
                    );

                    $this->info("  Point earning created successfully: " . $item->pelanggan_id);
                    $this->info("  Points: " . $poin);
                } catch (\Throwable $e) {
                    $this->error("  Error: " . $e->getMessage());
                    if (str_contains($e->getMessage(), 'Duplicate')) {
                        continue;
                    }
                    throw $e;
                }
            }
        });
        // =========================END CALCULATING POIN CUSTOMER================================
        $endTime = microtime(true);
        $duration = $endTime - $startTime;

        $this->output->newLine();
        $this->info("========================================================================");
        $this->info("  POIN CUSTOMER CALCULATED SUCCESSFULLY  ");
        $this->info("  Total Execution Time: " . number_format($duration, 2) . " seconds");
        $this->info("========================================================================");
    }
}